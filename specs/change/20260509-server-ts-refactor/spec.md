...
- [x] 记录 route ownership 规则
  - [x] 实现：把 route 和分区规则加入 `AGENTS.md`。
  - [x] 验证：确认新规则符合 root/app 边界，分区保持通用，并且不使用数字分区标签。
- [x] 抽取共享 server context
  - [x] 实现：新增 `server-context.ts`，承载 route registrar 需要的基础设施依赖。
  - [x] 实现：在 `server.ts` 中创建 context，保持现有 runtime value 不变。
  - [x] 验证：运行覆盖 startup-adjacent helper 的 daemon tests。
- [x] 抽取低风险 static resource routes
  - [x] 实现：把 skills、design systems、prompt templates、codex pets、frames 等只读 route registration 移到 `static-resource-routes.ts`。
  - [x] 验证：运行覆盖 resource route helper 的 daemon tests，然后运行 `pnpm --filter @open-design/daemon test`。
- [x] 抽取 project 相关 routes
  - [x] 实现：把 project、file、tab、conversation 和 template endpoint registration 移到 `project-routes.ts`，保持 route order。
  - [x] 验证：运行 projects/files/conversations/templates 相关 daemon tests。
- [x] 抽取 live artifact routes
  - [x] 实现：把 live artifact、preview、refresh、code 和 preview-comment endpoints 移到 `live-artifact-routes.ts`。
  - [x] 验证：运行 live artifact 和 preview-comment daemon tests。
- [x] 抽取 media routes
  - [x] 实现：把 media generation、media task 和 media config endpoints 移到 `media-routes.ts`。
  - [x] 验证：运行 media config/tasks 相关 daemon tests。
- [x] 抽取 MCP 和 integration routes
  - [x] 实现：把 MCP config、install info、token 和 OAuth endpoints 移到 `mcp-routes.ts`。
  - [x] 实现：保留现有 connector route registration 模式，并把它放在 Integrations 分区。
  - [x] 验证：运行 MCP config/OAuth/token 相关 daemon tests。
- [x] 抽取 deploy 和 import/export routes
  - [x] 实现：把 deploy endpoints 移到 `deploy-routes.ts`。
  - [x] 实现：把 import/export/finalize endpoints 移到 `import-export-routes.ts`。
  - [x] 验证：运行 deployment/import/export 相关 daemon tests。
- [x] 最后抽取 runtime、agents 和 chat routes
  - [x] 实现：在低风险 route groups 稳定后，把 chat、agent streaming、run lifecycle 和 runtime endpoints 移到 `chat-routes.ts`。
  - [x] 验证：运行覆盖 chat、agent spawning helpers、SSE events、stop behavior 和 run status 的 daemon tests。
- [ ] 最终验证
  - [x] 验证：运行 `pnpm guard`。
  - [x] 验证：运行 `pnpm typecheck`。
  - [ ] 验证：运行 `pnpm --filter @open-design/daemon test`。
  - [x] 验证：如果 route extraction 改动 package build inputs，运行 `pnpm --filter @open-design/daemon build`。
...
- `apps/AGENTS.md` - added daemon route ownership and stable semantic section rules.
- `apps/daemon/src/server.ts` - kept daemon bootstrap, middleware, core routes, static mounts, and top-level semantic route composition.
- `apps/daemon/src/server-context.ts` - added shared registrar/context typing surface.
- `apps/daemon/src/mcp-routes.ts` - moved MCP install-info, server config, token, and OAuth route registration.
- `apps/daemon/src/static-resource-routes.ts` - moved agents, skills, design systems, prompt templates, codex pets, and resource preview routes.
- `apps/daemon/src/project-routes.ts` - moved project workspace, conversations, comments, tabs, templates, artifacts, file, and upload routes.
- `apps/daemon/src/live-artifact-routes.ts` - moved live artifact tool/UI, preview, refresh, code, and event routes.
- `apps/daemon/src/deploy-routes.ts` - moved deploy config, deploy, preflight, deployment list, and deployment check routes.
- `apps/daemon/src/import-export-routes.ts` - moved Claude ZIP import, folder import, archive/export, and finalize routes.
- `apps/daemon/src/media-routes.ts` - moved media config, media generation task, media status/result, and research routes.
- `apps/daemon/src/chat-routes.ts` - moved chat, run status/SSE, connection test, critique interrupt, and proxy routes.
- New route files use TypeScript checking; no new `*-routes.ts` file uses `// @ts-nocheck`.

### Verification

- Passed: `pnpm --filter @open-design/daemon typecheck`.
- Passed: `pnpm --filter @open-design/daemon build`.
- Passed: `pnpm guard`.
- Passed: `pnpm --filter @open-design/desktop build && pnpm typecheck`.
- `pnpm --filter @open-design/daemon test` ran route suites successfully after fixing startup context, with remaining failures in `finalize-design.test.ts` artifact-name assertions and `project-watchers.test.ts` chokidar timeout.
